...
| # | Where | Do | Say | Paste |
|---|-------|----|-----|-------|
| 3.1 | `pom.xml` | Show the `spring-ai-starter-mcp-server-webmvc` dependency. Then show the `spring-ai-bom` import and the `spring-ai.version` value `2.0.0`. | "This is the Todo sample from the description, cloned and open here with GitHub Copilot signed in. It already has working Todo operations, and we'll expose them as MCP tools. In `pom.xml`, we'll show Spring AI's WebMVC MCP server starter, then the imported Bill of Materials, BOM for short, and its `2.0.0` version. The starter supplies the MCP server support; the BOM keeps every Spring AI dependency on the same version. Now let's look at the Java methods the application publishes." | — |
...
| # | Where | Do | Say | Paste |
|---|-------|----|-----|-------|
| 4.1 | VS Code — Activity Bar → Extensions | Select the **Extensions** button in the Activity Bar. Search **`@mcp playwright`** and open the Playwright MCP server result. Review its publisher and command configuration. Select **Install**, confirm trust for the server, and wait for its running status. | "This is the Todo sample from the description, cloned and open here with GitHub Copilot signed in. To test the page through a real browser, Copilot needs browser-control tools, so we'll open Extensions from the Activity Bar, search the MCP server gallery for `@mcp playwright`, and open the Playwright result. Before installing, we'll review the publisher and the command this server runs. We'll select Install, confirm we trust this server, and wait for its running status. With the browser tools in place, we'll look at the page elements they will target." | `@mcp playwright` |
| 4.2 | `templates/index.html` | Show `data-testid="new-todo-input"`, `data-testid="add-todo"`, `data-testid="todo-item"`, `th:attr="aria-label='Toggle ' + ${todo.title}"`, and `data-testid="delete-todo"` in the template. | "In `index.html`, the input, the add button, every row, and every delete button all carry stable test ids, and each checkbox builds an accessible `Toggle` label from that Todo's title. These ids identify the exact controls without depending on visual styling. With them in place, we'll first confirm the existing Java tests still pass." | — |
